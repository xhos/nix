...
# tested on 20.2.3, nixpkgs rev 4652ba995a945108fb891191c1e910b9a6ed9064
{
  ...
  ffmpeg-encoder-plugin = pkgs.stdenv.mkDerivation (finalAttrs: {
    ...
    nativeBuildInputs = with pkgs; [cmake ffmpeg];
  ...
  ioPlugins = [ffmpeg-encoder-plugin aac-codec-plugin];
  ...
  davinci-resolve-studio-cracked = let
    davinci-patched = pkgs.davinci-resolve-studio.davinci.overrideAttrs (old: {
      ...
      postInstall = ''
        ...
        ${lib.getExe pkgs.perl} -0777 -pi -e 's/(\x40\x84\xED)\x74(.\xBF\x16\x00\x00\x00\xBE.\x01\x00\x00\xE8)/$1\x75$2/g' $out/bin/resolve

        # Write license file into store
        ...
        # IOPlugins dir must exist for bwrap bind mount
    ...
    pkgs.buildFHSEnv {
      ...
      targetPkgs = pkgs:
        with pkgs;
          ...
          ++ [davinci-patched]
          ++ (lib.concatLists (map (drv: drv.runtimeDependencies or []) ioPlugins));
      ...
      extraBwrapArgs = [
        ...
        "--ro-bind ${ioPluginsMerged}/IOPlugins ${davinci-patched}/IOPlugins"
      ];
      ...
      passthru = {
        ...
        updateScript = lib.getExe (
          pkgs.writeShellApplication {
            ...
            runtimeInputs = [pkgs.curl pkgs.jq pkgs.common-updater-scripts];
            text = ''
              set -o errexit
